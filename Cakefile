...
# Convert a command to run in Windows
convertToWindowsCmd = (cmd) ->
    cmd = cmd.split ' '
    cmd[0] = 'node_modules\\.bin\\' + (do (v = cmd[0].split '/') -> v[v.length - 1]) + '.cmd'
    cmd.join ' '
...
# code.
run = (cmd, callback) ->
    cmd = convertToWindowsCmd cmd if os.platform() == 'win32'
    exec cmd, (err, stdout) ->
        ...
        do (key, files) ->
            ...
            run "./node_modules/coffeelint/bin/coffeelint -f test/lint.json #{files}", ->
                ...
                run "./node_modules/coffee-script/bin/coffee -j lib/#{key}.js --compile #{files}", ->
                    console.log "Building dist/#{key}.min.js"
                    run "./node_modules/uglify-js/bin/uglifyjs --comments --compress --mangle -o dist/#{key}.min.js lib/#{key}.js"
